...
public class BookDao {
	...
	public Book getBooksBySerialNumber(Connection conn, int srNo) throws SQLException {
	...
	public Book getBooksByAuthorName(Connection conn, String authorName) throws SQLException {
		...
		try(PreparedStatement ps = conn.prepareStatement(query)){
			...
			try(ResultSet rs = ps.executeQuery()){
				...
				Book book = new Book();
				book.setAuthorName(rs.getString("author_name"));
				book.setBookName(rs.getString("author_name"));
				book.setBookQty(rs.getInt("author_name"));
				book.setId(rs.getInt("id"));
				book.setSrNo(rs.getInt("sr_no"));
				return book;
				}
